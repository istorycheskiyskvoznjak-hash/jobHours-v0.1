...
import type { WorkLog, User, UserRole } from '../types';
import { StatsSummary } from './StatsSummary';
import { HoursChart } from './HoursChart';
import { CalendarView } from './CalendarView';
import { UserIcon, TeamIconDetailed, ClockIcon, ChartBarIcon, TeamIconWithHelmets, SearchIcon, IdentificationIcon } from './IconComponents';
...
const UserManagement: React.FC<{showToast: (message: string) => void}> = ({ showToast }) => {
    ...
    const fetchUsers = useCallback(async () => {
        ...
        setInitialUsers(JSON.parse(JSON.stringify(mappedUsers)));
        setIsLoading(false);
    }, [showToast]);
    ...
    const handleFieldChange = (id: string, field: keyof User, value: string | UserRole | number) => {
        setUsers(prev => prev.map(user => user.id === id ? { ...user, [field]: value } : user));
    };

    const handleSaveChanges = async () => {
        const changedUsers = users.filter(user => 
            JSON.stringify(user) !== JSON.stringify(initialUsers.find(u => u.id === user.id))
        );
        ...
        if (error) {
            ...
            showToast('Данные пользователей успешно обновлены');
            fetchUsers();
        }
    };

    const roles: UserRole[] = ['Worker', 'Supervisor', 'Administrator'];

    if (isLoading) {
        return (
            <div className="flex justify-center items-center p-10">
                <ClockIcon className="h-10 w-10 text-primary animate-spin" />
            </div>
        )
    }
        ...
        <div className="bg-card shadow-lg p-6 rounded-3xl">
            <div className="flex justify-between items-center mb-6">
                ...
                <button
                    ...
                    className="font-semibold py-2 px-5 border border-transparent rounded-xl text-white bg-primary hover:bg-primary-darker focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-card focus:ring-primary transition-colors"
                >
                    Сохранить изменения
                </button>
            </div>
            ...
            <div className="overflow-x-auto">
                <table className="min-w-full divide-y divide-line">
                    <thead className="bg-surface/50">
                        <tr>
                            <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-text-muted uppercase tracking-wider">Пользователь</th>
                            <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-text-muted uppercase tracking-wider">Имя</th>
                            <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-text-muted uppercase tracking-wider">Фамилия</th>
                            <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-text-muted uppercase tracking-wider">Компания</th>
                            <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-text-muted uppercase tracking-wider">Роль</th>
                        </tr>
                    </thead>
                        ...
                        {users.map((user) => (
                            <tr key={user.id}>
                                <td className="px-6 py-4 whitespace-nowrap">
                                    <div className="text-sm font-medium text-text-heading">{user.username}</div>
                                </td>
                                 <td className="px-6 py-4 whitespace-nowrap">
                                    <input
                                        type="text"
                                        ...
                                        className="w-full px-3 py-1.5 border border-line rounded-xl focus:ring-1 focus:ring-primary focus:border-primary bg-surface text-text-body"
                                    />
                                </td>
                                 <td className="px-6 py-4 whitespace-nowrap">
                                    <input
                                ...
                                <td className="px-6 py-4 whitespace-nowrap">
                                    <input
                                        ...
                                        onChange={(e) => handleFieldChange(user.id, 'companyName', e.target.value)}
                                        className="w-full px-3 py-1.5 border border-line rounded-xl focus:ring-1 focus:ring-primary focus:border-primary bg-surface text-text-body"
                                    />
                                </td>
                                <td className="px-6 py-4 whitespace-nowrap">
...
export const AdminDashboard: React.FC<AdminDashboardProps> = ({ 
    ...
    const TabButton: React.FC<{ tabView: 'team' | 'users', label: string, shortLabel?: string, icon: React.ReactNode }> = ({ tabView, label, shortLabel, icon }) => (
        <button
            onClick={() => setView(tabView)}
            className={`flex items-center gap-2 px-4 py-3 text-base font-bold rounded-t-lg border-b-2 transition-colors ${
                view === tabView 
                ? 'text-primary border-primary' 
                : 'text-text-muted border-transparent hover:text-text-body hover:border-line'
            }`}
        >
            {icon}
            {shortLabel ? (
        ...
        <div>
            <div className="border-b border-line mb-6">
                <nav className="-mb-px flex space-x-2 sm:space-x-4">
                    <TabButton tabView="team" label="Обзор команды" shortLabel="Команда" icon={<SearchIcon className="h-6 w-6"/>} />
                    <TabButton tabView="users" label="Управление" shortLabel="Роли" icon={<IdentificationIcon className="h-6 w-6" />} />
                </nav>
            </div>
            {renderContent()}
        </div>
